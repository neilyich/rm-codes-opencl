...
#ifndef UNTITLED_LINEARCOEFSEXTRACTOR_H
#define UNTITLED_LINEARCOEFSEXTRACTOR_H


#include "OpenCLExecutor.h"

class LinearCoefsExtractor : public OpenCLExecutor {
public:
    explicit LinearCoefsExtractor(const std::string& kernelCode, const std::string& name) : OpenCLExecutor(kernelCode, name) {}

    int* extract(int* target, int targetNbits, int* source, int sourceNbits) {
        ...
        exitCode |= queue.enqueueNDRangeKernel(kernel, cl::NDRange(1), cl::NDRange(targetNbits - 1));
...
#endif //UNTITLED_LINEARCOEFSEXTRACTOR_H
